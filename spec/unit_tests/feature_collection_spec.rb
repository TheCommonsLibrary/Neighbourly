...
describe 'FeatureCollection' do
	...
	describe '#to_a' do
		...
		before :each do
			db[:claims].insert(:mesh_block_slug => 'slug1', :mesh_block_claimer => 'user 1', :claim_date => Time.now)
			db[:claims].insert(:mesh_block_slug => 'slug2', :mesh_block_claimer => 'user 2', :claim_date => Time.now)
		end
		...
		it 'should convert raw query data into correct mash block format' do
			feature_collection = FeatureCollection.new query_result, 'user 1', mesh_block_claimers
